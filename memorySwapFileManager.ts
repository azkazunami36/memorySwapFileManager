...
export class MemorySwapFileManager {
    constructor(console: {
        trace: (message: string) => void;
        debug: (message: string) => void;
        info: (message: string) => void;
        ...
        error: (message: string) => void;
        fatal: (message: string) => void;
    }) { consol = console; this.#readyfunc = this.init(); }
    busy: Promise<void> | undefined; // メモリの操作中かどうか
    #readyfunc;
    #ready = false;
    async ready() {
    ...
    async init() {
        ...
        this.#interval = setInterval(async () => {
            ...
            this.busy = this.checkMemoryStatus();
            await this.busy;
            this.busy = undefined;
        }, 500);
    }
    ...
    swapFile = {
        ...
        release: async (fileName?: string) => {
            ...
            try {
                ...
                if (!swapStatus || !memoryStatus) return false;
